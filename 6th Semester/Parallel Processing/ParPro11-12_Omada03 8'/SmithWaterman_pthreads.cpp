...
void *calculate_H(void *arg)
{
 float temp[4];
 unsigned int id = (int) arg;;
 unsigned short plithos=0;
 unsigned short num_of_thr;
 unsigned short k;
 unsigned short s=1;
 ...
 for(unsigned short j=1; s<=N_a; ) {
	...
	if(plithos<THR_NUMB) num_of_thr=plithos;
	else num_of_thr=THR_NUMB;
	if (id < num_of_thr) {
		int start,end;//arxi kai telos kathe thread sti diagwnio
        	start=s+id*(plithos/num_of_thr);
		if (id==num_of_thr-1) end=s+plithos; //to teleytaio thread oti perisepsei
		else end=s+(id+1)*(plithos/num_of_thr);
		k=j-id*(plithos/num_of_thr);
		...
		for(unsigned short i=start; i<end; i++) {
			 ...
			 //cout <<"Thread:"<<id<<", to i einai " << i << ", to k "<< k<< " to end " << end <<endl;
        	   	 H[i][k] = max;
        	   	 switch(ind) {
...
int main(int argc, char** argv) {
    ...
    pthread_t thread_id[THR_NUMB];
    pthread_barrier_init(&barrier,NULL,THR_NUMB);
    ...
    for(int i=0; i<THR_NUMB; i++){
		pthread_create(&thread_id[i], NULL, calculate_H, (void *) i);
    }
    
    for(unsigned short i=0; i<THR_NUMB; i++){
